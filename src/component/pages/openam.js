...
const { REACT_APP_OAUTH_PROVIDER_URL, REACT_APP_CLIENT_ID, REACT_APP_CLIENT_SECRET, REACT_APP_CLIENT_STATE } = process.env;
...
export default function OpenAM({setAccessToken = f => f, error, setError = f => f}) {
   ...
   function redirectToOpenAM() {
      const state = REACT_APP_CLIENT_STATE || 'af0ifjsldkj';
      const qParams = [
         `response_type=code`,
      ...
      window.location.assign(`${REACT_APP_OAUTH_PROVIDER_URL}/authorize?${qParams}`);
   }
